...
namespace SD.FrameWork.UI.Pager
{
    /// <summary>
    ...
    public class Pager : IHtmlString
    {
        ...
        private string _paginationFormat = "当前 {0}项 - {1}项， 共 {2} 项";
        private string _paginationSingleFormat = "当前 {0} 共 {1} ";
        ...
        public string ToHtmlString()
        {
            ...
            builder.Append("<div class=\"email-footer clearfix\">");

            RenderLeftSideOfPager(builder);

            builder.Append("<ul class='pagination m-t-0 m-b-0 pull-right'>");
        ...
        {
            builder.Append("<span class='hidden-sm hidden-xs'>");
        ...
        protected virtual void RenderRightSideOfPager(StringBuilder builder)
        {
            ...
            {
                builder.AppendFormat("<li class=\"disabled\"><a>{0}</a></li>",_paginationFirst);
            }
            else
            {
                builder.AppendFormat("<li>{0}</li>",CreatePageLink(1, _paginationFirst));
            }
            ...
            {
                builder.AppendFormat("<li>{0}</li>", CreatePageLink(_pagination.PageNumber - 1, _paginationPrev));
            }
            else
            {
                builder.AppendFormat("<li class=\"disabled\"><a>{0}</a></li>", _paginationPrev);
            }

            RenderPagingLinks(builder);
            ...
            {
                builder.AppendFormat("<li>{0}</li>", CreatePageLink(_pagination.PageNumber + 1, _paginationNext));
            }
            else
            {
                builder.AppendFormat("<li class=\"disabled\"><a>{0}</a></li>", _paginationNext);
            }
            ...
            {
                builder.AppendFormat("<li>{0}</li>",CreatePageLink(lastPage, _paginationLast));
            }
            else
            {
                builder.AppendFormat("<li class=\"disabled\"><a>{0}</a></li>", _paginationLast);
            }
        ...
        protected virtual void RenderPagingLinksRange(int start, int end, StringBuilder builder)
        {
            for (int i = start; i < end; i++)
            {
                if (_pagination.PageNumber == i + 1)
                {
                    //builder.Append(CreatePageLink(i+1,(i+1).ToString(),"current_index"));
                    builder.AppendFormat("<li class='active hidden-sm hidden-xs'><a>{0}</a></li>", (i + 1).ToString());
                }
                else
                {
                    //builder.Append(CreatePageLink(i + 1, (i + 1).ToString()));
                    builder.AppendFormat("<li class='hidden-sm hidden-xs'>{0}</li>", CreatePageLink(i + 1, (i + 1).ToString()));
                }
            }
        }

        protected virtual void RenderPagingLinks(StringBuilder builder)
        {
            //builder.Append("<span class='paginationNumbers'>");

            if (_pagination.TotalPages < _listIndexCount)
            {
                RenderPagingLinksRange(0, _pagination.TotalPages, builder);
            }
            else
            {
                int lowerBound = (_pagination.PageNumber - _listIndexCount / 2);
                int upperBound = (_pagination.PageNumber + _listIndexCount / 2 + 1);

                if (_pagination.PageNumber <= _listIndexCount / 2)
                {
                    RenderPagingLinksRange(0, _listIndexCount, builder);
                }
                else if (_pagination.PageNumber >= (_pagination.TotalPages - _listIndexCount / 2))
                {
                    RenderPagingLinksRange((_pagination.TotalPages - _listIndexCount), _pagination.TotalPages, builder);
                }
                else
                {
                    RenderPagingLinksRange(lowerBound - 1, upperBound - 1, builder);
                }
            }
