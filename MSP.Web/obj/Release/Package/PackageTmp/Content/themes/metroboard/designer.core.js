...
var Designer = {
    ...
    initialize: {
        ...
        initShapes: function () {
            ...
            $("#shape_panel").append('<div id="export_button" class="layout"><a href="javascript:;" class="button" onclick="export2svg()">保存</a></div>');
    ...
    op: {
        ...
        shapeDraggable: function () {
            ...
            b.bind("mousedown.drag", function (r) {
                ...
                j = j.concat(n);
                var m = Utils.getAttachedShapes(j);
                j = j.concat(m);
                var p = Utils.getCollapsedShapes(j);
                j = j.concat(p);
                var e = [];
                if (g) {
                    for (var l = 0; l < j.length; l++) {
                        var o = j[l];
                        if (o.name == "linker") {
                            if (o.from.id && e.indexOf(o.from.id) < 0) {
                                e.push(o.from.id)
                            }
                            if (o.to.id && e.indexOf(o.to.id) < 0) {
                                e.push(o.to.id)
                            }
                        }
                        if (e.indexOf(o.id) < 0) {
                ...
                a.bind("mousemove.drag", function (w) {
                    UI.hideShapeOptions();
                    var v = Utils.getRelativePos(w.pageX, w.pageY, b);
                    var y = {
                        x: v.x - f.x,
                        y: v.y - f.y
                    };
        ...
        editShapeText: function (a, g) {
            ...
            var f = a.getTextBlock();
            var h = 0;
            if (g) {
                g.x = g.x.restoreScale();
                g.y = g.y.restoreScale();
                if (a.props.angle != 0) {
                    var r = {
                        x: a.props.x + a.props.w / 2,
                        y: a.props.y + a.props.h / 2
                    };
                    g = Utils.getRotated(r, g, -a.props.angle)
                }
                var e = g.x - a.props.x;
                var d = g.y - a.props.y;
                for (var n = 0; n < f.length; n++) {
                    var k = f[n];
                    if (Utils.pointInRect(e, d, k.position)) {
                        h = n;
                        break
                    }
                }
            ...
            Designer.contextMenu.hide();
            var m = $("#shape_text_edit");
            if (m.length == 0) {
                m = $("<textarea id='shape_text_edit'></textarea>").appendTo("#designer_canvas")
            }
            var l = $("#shape_text_ruler");
            if (l.length == 0) {
                l = $("<textarea id='shape_text_ruler'></textarea>").appendTo("#designer_canvas")
            }
            $(".text_canvas[forshape=" + a.id + "][ind=" + h + "]").hide();
            var q = f[h];
            var s = a.textBlock[h];
            var b = Utils.getShapeFontStyle(a.fontStyle);
            b = $.extend({}, b, q.fontStyle);
            var o = q.position;
            ...
            $("#shape_text_edit").unbind().bind("keyup", function () {
                ...
                Designer.op.shapeChangeHeight(a, h, u, B);
                var v = {
                    x: o.x + o.w / 2,
                    y: o.y + o.h / 2
                };
                ...
                var t = Math.round(z / (Math.PI * 2) * 360);
                var x = "rotate(" + t + "deg) scale(" + Designer.config.scale + ")";
                m.css({
                    width: o.w,
                    height: D,
                    "padding-top": y,
                    left: A.x.toScale() - o.w / 2 - 2,
                ...
                if (v.keyCode == 13 && v.ctrlKey) {
                    ...
                    if (v.keyCode == 27) {
                        i.unbind().remove();
                        $(".text_canvas[forshape=" + a.id + "][ind=" + h + "]").show()
                    } else {
                        if (v.keyCode == 66 && v.ctrlKey) {
                            var t = !b.bold;
                            if (a.textBlock.length == 1) {
                                a.fontStyle.bold = t
                            } else {
                                s.fontStyle = $.extend(s.fontStyle, {
                                    bold: t
                                })
                            }
                            b.bold = t;
                            Model.update(a);
                            var u = t ? "bold" : "normal";
                            $(this).css("font-weight", u);
                            l.css("font-weight", u);
                            UI.update();
                            v.preventDefault()
                        } else {
                            if (v.keyCode == 73 && v.ctrlKey) {
                                var t = !b.italic;
                                if (a.textBlock.length == 1) {
                                    a.fontStyle.italic = t
                                } else {
                                    s.fontStyle = $.extend(s.fontStyle, {
                                        italic: t
                                    })
                                ...
                                b.italic = t;
                                Model.update(a);
                                var u = t ? "italic" : "normal";
                                $(this).css("font-style", u);
                                l.css("font-style", u);
                                UI.update();
                                v.preventDefault()
                            } else {
                                if (v.keyCode == 85 && v.ctrlKey) {
                                    var t = !b.underline;
                                    if (a.textBlock.length == 1) {
                                        a.fontStyle.underline = t
                                    } else {
                                        s.fontStyle = $.extend(s.fontStyle, {
                                            underline: t
                                        })
                                    ...
                                    b.underline = t;
                                    Model.update(a);
                                    var u = t ? "underline" : "none";
                                    $(this).css("text-decoration", u);
                                    l.css("text-decoration", u);
                                    v.preventDefault();
                                    UI.update();
                                    v.preventDefault()
                                }
        ...
        shapeChangeHeight: function (shape, index, textH, text) {
            var old_text = shape.textBlock[index].text;
            if (old_text == text) {
                return
            }
            if (textH < 30) {
            ...
            if (shape.name == "interface") {
                if (index == 1) {
                    shape.props.heights[1] = textH;
                    shape.props.h = shape.props.heights[0] + shape.props.heights[1] + 25;
                    shape.textBlock[1].position.h = shape.props.h - shape.props.heights[0] - 10
                } else {
                    if (index == 0) {
                        if (isNaN(shape.props.heights[1])) {
                            var tb = shape.textBlock[1].position.h;
                            shape.props.heights[1] = eval(tb.replace("h", shape.props.h))
                        }
                        shape.props.heights[0] = textH;
                        shape.path[1].actions[0].y = textH;
                        shape.path[1].actions[1].y = textH;
                        shape.textBlock[0].position.h = textH;
                        shape.props.h = shape.props.heights[0] + shape.props.heights[1];
                        shape.textBlock[1].position.h = shape.props.h - textH;
                        shape.textBlock[1].position.y = textH + 3
                    }
                }
                ...
                if (shape.name == "cls") {
                    if (index == 0) {
                        shape.props.heights[0] = textH;
                        if (isNaN(shape.props.heights[2])) {
                            var tb = shape.textBlock[2].position.h;
                            shape.props.heights[2] = eval(tb.replace("h", shape.props.h))
                        }
                        if (isNaN(shape.props.heights[1])) {
                            var tb = shape.textBlock[1].position.h;
                            shape.props.heights[1] = eval(tb.replace("h", shape.props.h))
                        ...
                        shape.path[1].actions[1].y = textH;
                        shape.textBlock[0].position.h = textH;
                        shape.props.h = shape.props.heights[0] + shape.props.heights[1] + shape.props.heights[2] + 20;
                        shape.textBlock[1].position.y = textH;
                        shape.textBlock[1].position.h = (shape.props.h - textH) / 2;
                        shape.textBlock[2].position.y = textH + shape.textBlock[1].position.h;
                        shape.textBlock[2].position.h = (shape.props.h - textH) / 2;
                        shape.path[1].actions[2].y = textH + shape.props.heights[1] + 10;
                        shape.path[1].actions[3].y = textH + shape.props.heights[1] + 10
                    }
                    if (index == 1) {
                        shape.props.heights[1] = textH + 3;
                        shape.textBlock[1].position.h = textH;
                        if (isNaN(shape.props.heights[2])) {
                            var tb = shape.textBlock[2].position.h;
                            shape.props.heights[2] = eval(tb.replace("h", shape.props.h))
                        }
                        shape.props.h = shape.props.heights[0] + shape.props.heights[1] + shape.props.heights[2] + 20;
                        shape.textBlock[1].position.y = shape.props.heights[0] + 3;
                        shape.textBlock[2].position.y = textH + shape.props.heights[0] + 3;
                        shape.textBlock[2].position.h = shape.props.h - textH - shape.props.heights[0];
                        shape.path[1].actions[2].y = shape.props.heights[0] + textH + 6;
                        shape.path[1].actions[3].y = shape.props.heights[0] + textH + 6
                    } else {
                        if (index == 2) {
                            shape.props.heights[2] = textH;
                            shape.textBlock[2].position.h = textH;
                            if (isNaN(shape.props.heights[1])) {
                                var tb = shape.textBlock[1].position.h;
                                shape.props.heights[1] = eval(tb.replace("h", shape.props.h))
                            }
                            shape.props.h = shape.props.heights[0] + shape.props.heights[1] + shape.props.heights[2] + 17;
                            shape.textBlock[2].position.y = shape.props.heights[0] + shape.props.heights[1] + 13;
                            shape.path[1].actions[2].y = shape.props.heights[0] + shape.props.heights[1] + 8;
                            shape.path[1].actions[3].y = shape.props.heights[0] + shape.props.heights[1] + 8;
                            shape.textBlock[1].position.h = shape.props.h - shape.props.heights[0] - textH - 20
                        }
                    }
                    Schema.initShapeFunctions(shape)
                }
        ...
        canvasDraggable: function () {
            var a = $("#canvas_container");
            a.css("cursor", "url(/Content/themes/metroboard/images/cursor_hand.png) 8 8, auto");
    ...
    painter: {
        ...
        renderShape: function (i) {
            ...
            if (k.length == 0) {
                var c = $("#designer_canvas");
                k = $("<div id='" + i.id + "' class='shape_box'><div class='shape_info' style='display:none'></div><canvas class='shape_canvas'></canvas></div>").appendTo(c)
            }
            if (i.attribute && i.attribute.collapseBy) {
                k.hide()
            } else {
                k.show()
            }
            var e = Utils.getShapeBox(i);
            var b = (e.w + 20).toScale();
            var f = (e.h + 20).toScale();
            k.find(".shape_canvas").attr({
                width: b,
                height: f
            });
            k.css({
            ...
            var deg =Math.floor( i.props.angle * 180 / Math.PI);
            if (deg == 0 || deg == 180) {
                k.find(".shape_info").attr({
                    shape: i.shape,
                    width: e.w,
                    height: e.h,
                    x: e.x,
            ...
            k.find(".shape_info").attr({
                ...
                fillcolor:(typeof i.fillStyle.color) == "undefined" ? "255,255,255" : i.fillStyle.color,
                filltype: i.fillStyle.type,
                linewidth: i.lineStyle.width,
                name: i.name,

            });
            var l = k.find(".shape_canvas")[0].getContext("2d");
            l.clearRect(0, 0, i.props.w + 20, i.props.h + 20);
            l.scale(Designer.config.scale, Designer.config.scale);
            l.translate(10, 10);
            l.translate(i.props.x - e.x, i.props.y - e.y);
            l.translate(i.props.w / 2, i.props.h / 2);
            l.rotate(i.props.angle);
            l.translate(-(i.props.w / 2), -(i.props.h / 2));
            l.globalAlpha = i.shapeStyle.alpha;
            l.lineJoin = "round";
            this.renderShapePath(l, i, false, function () {
                var o = i.id;
                var p = Model.getShapeById(o);
                Designer.painter.renderShape(p)
            });
            this.renderMarkers(l, i);
            var n = i.getPath();
            var j = Utils.copy(n[n.length - 1]);
            j.fillStyle = {
                type: "none"
            };
            j.lineStyle = {
            ...
            var d = [j];
            this.renderPath(l, i, d);
            this.renderText(i, e);
            this.renderDataAttributes(i, e);
            k.children(".shape_comment_ico").remove();
            if (showCommentIco) {
                var h = false;
                if (Model.comments && Model.comments.length > 0) {
                    for (var m = 0; m < Model.comments.length; m++) {
                        var a = Model.comments[m];
                        if (a.shapeId == i.id) {
                            h = true;
                            break
                        }
                ...
                if (h) {
                    var g = $("<div class='shape_comment_ico'></div>").appendTo(k);
                    g.bind("mousedown", function (o) {
                        o.stopPropagation();
        ...
        fillShape: function (c, a, b, e) {
            ...
            var div = $("#" + c.id);
            div.find(".shape_info").attr({
                fillcolor:a.fillStyle,
                filltype: b.type,
                alpha: a.globalAlpha,
